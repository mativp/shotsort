...
function findCaptureTimeInCiffHeap(byteSource, heapStart, heapEnd, isLittleEndian, howDeep) {
  ...
  const directoryStart = heapStart + readUInt32At(byteSource, directoryPointerAt, isLittleEndian);
  const entryCount = readUInt16At(byteSource, directoryStart, isLittleEndian);
  const entriesStart = directoryStart + BYTES_IN_A_CIFF_ENTRY_COUNT_FIELD;
  const theEntriesFitBeforeThePointer = entriesStart + entryCount * BYTES_PER_CIFF_DIRECTORY_ENTRY <= directoryPointerAt;
  if (!theEntriesFitBeforeThePointer || entryCount > MOST_ENTRIES_A_REAL_CIFF_DIRECTORY_HAS) return null;

  for (let entryIndex = 0; entryIndex < entryCount; entryIndex++) {
    const entryStart = entriesStart + entryIndex * BYTES_PER_CIFF_DIRECTORY_ENTRY;
    ...
    const valueStart = heapStart + readUInt32At(byteSource, entryStart + 6, isLittleEndian);
    const valueEnd = valueStart + valueLength;
    if (valueEnd > heapEnd) continue;
    ...
    if ((tag & CIFF_DATA_TYPE_MASK) === CIFF_DATA_TYPE_SUBDIRECTORY) {
      ...
      const clock = onlyIfPlausible(cameraClockFromSecondsSince1970(readUInt32At(byteSource, valueStart, isLittleEndian)));
      if (clock !== null) return clock;
    }
  }
  return null;
