...
const readBy = (formatName, fixtures) => fixtures.map((each) => ({ ...each, readBy: formatName }));

// Each fixture listed here is read whole, cut short, corrupted and put to exiftool, so a
// format added to the list is checked all four ways without touching a test.
export const everyFixtureFormatIsBuiltFrom = () => [
  ...
  ...readBy('TIFF and the raw dressed up as one', [
  ...
  ...readBy('RIFF, which is AVI and WebP', [
